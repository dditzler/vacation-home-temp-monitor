...
// ESP32-C6: use IDF low-level GPIO functions which correctly handle LP GPIOs (GPIO0-GPIO7)
#include "hal/gpio_ll.h"
#include "esp_rom_sys.h"
// Arduino's delayMicroseconds() may be miscalibrated on RISC-V C6.
// esp_rom_delay_us() is in ROM and always accurate regardless of CPU frequency.
...
{
    return (IO_REG_TYPE)gpio_ll_get_level(&GPIO, (gpio_num_t)pin);
}

static inline __attribute__((always_inline))
void directWriteLow(IO_REG_TYPE pin)
{
    gpio_ll_set_level(&GPIO, (gpio_num_t)pin, 0);
}
...
#undef  noInterrupts
#endif
// Static spinlock avoids per-call stack allocation inside timing-critical bit ops
static portMUX_TYPE _ow_spinlock = portMUX_INITIALIZER_UNLOCKED;
#define noInterrupts() portENTER_CRITICAL(&_ow_spinlock)
#define interrupts() portEXIT_CRITICAL(&_ow_spinlock)
